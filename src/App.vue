...
export default defineComponent({
  ...
  methods: {
    ...
    async getUser(name: string) {
      console.log(name)
      const response = await axios.post("http://34.60.100.110/get_user", {
        name: name
      }, {
    ...
    async getUsers() {
      const response = await fetch("http://34.60.100.110/get_users");
    ...
    async addUser(name: string) {
      const response = await axios.post("http://34.60.100.110/save_user", {
        name: name
      }, {
        headers: { "Content-Type": "application/json" }
      });
    ...
    async sendSquad() {
      try {
        const response = await axios.post("http://34.60.100.110/save_squad", {
    ...
    async sendLog(action_log: ActionLog) {
      try {
        const response = await fetch('http://34.60.100.110/save_log', {
          method: 'POST',
          headers: {
    ...
    async delLog(id: number, name: string) {
      try {
        const response = await fetch('http://34.60.100.110/delete_log', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
    ...
    setupSSE() {
      ...
      this.eventSource = new EventSource('http://34.60.100.110/sse');
